...
class AboutIteration(Koan):
    ...
    def test_iterators_are_a_type(self):
        ...
        self.assertEqual(sum(range(1,6)), fib)

    def test_iterating_with_next(self):
        stages = iter(['alpha', 'beta', 'gamma'])

        try:
            self.assertEqual('alpha', next(stages)) # 1st time next() is called 
            next(stages)                            # 2nd time next() is called
            self.assertEqual('gamma', next(stages)) # 3rd time next() is called
            next(stages)                            # 4th time next() is called
    ...
    def test_reduce_will_blow_your_mind(self):
        ...
        #   reduce returns the single value result from invoking a method on elements
